...
//   // Socket.io Setup with Redis adapter for cross-worker communication
//   const io = require('socket.io')(server, {
//     cors: {
//       origin: process.env.FRONTEND_URL || "http://localhost:3000",
//       methods: ["GET", "POST"]
...
//   // Stock update namespace with authentication middleware
//   const stockNamespace = io.of('/stock-updates');
...
//       }
//       // Verify token using your existing auth system
//       const user = await require('./middleware/AuthMiddleware').verifyToken(token);
//       socket.user = user;
//       next();
//     } catch (err) {
...
//     socket.on('subscribe', ({ storeId, productId }) => {
//       const room = `store:${storeId}:product:${productId}`;
//       socket.join(room);
//       console.log(`Worker ${process.pid}: Client subscribed to ${room}`);
//     });
...
//       const room = `store:${storeId}:product:${productId}`;
//       socket.leave(room);
//       console.log(`Worker ${process.pid}: Client unsubscribed from ${room}`);
//     });
...
//   // Join room for specific store/product
//   socket.on('subscribe', ({ storeId, productId }) => {
//     const room = `store:${storeId}:product:${productId}`;
//     socket.join(room);
